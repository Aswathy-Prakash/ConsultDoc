...
const Main1 = (props) => {
 ...
 useEffect(() => {
  ...
  const handleSubmit = async (event) => { 
    ...
    const Regnom = event.target.regnom.value;
   ...
   const signature = await window.ethereum.request({
    ...
    createDoctor(Fullname, Email2, Regnom, Specialization, Upassword, signature);
  };

  const createDoctor = async (Fullname, Email2, Regnom, Specialization, Upassword, signature) => {
  ...
  return true;
    ...
    if (!isPasswordStrong) {
      ...
      const result = await CandRegContract.methods
        .createDoctor(Fullname, Email2, Regnom, Specialization, hashedPassword, signature)
        .send({ from: account });
  ...
  return (
    <>
      ...
      <TextField name="regnom" label="Registration Number" sx={{
          '& label': {
            color: 'teal', // Label color
          },
          '& .MuiOutlinedInput-root': {
        ...
        <TextField name="specialization" label="Specialization" sx={{
          '& label': {
            color: 'teal', // Label color
          },
          '& .MuiOutlinedInput-root': {
